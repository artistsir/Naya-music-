...
async def play_handler(_, m: types.Message):
    ...
    sent = await m.reply_text(message.lang["play_searching"])
    file = None
    ...
    if await db.get_call(m.chat.id):
        await sent.edit_text(
            message.lang["play_queued"].format(position, file.url, file.title, file.duration, m.from_user.mention),
            reply_markup=buttons.play_queued(m.chat.id, file.id, message.lang["play_now"])
        )
        return
...
async def skip_handler(_, m: types.Message):
    if not await db.get_call(m.chat.id):
        return await m.reply_text(message.lang["not_playing"])
...
async def ping_handler(_, m: types.Message):
    start = time.time()
    sent = await m.reply_text(message.lang["pinging"])
    ...
    await sent.edit_media(
        media=types.InputMediaPhoto(
            ...
            caption=message.lang["ping_pong"].format(
                latency,
                uptime_str,
                psutil.cpu_percent(),
